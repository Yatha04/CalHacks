...
import { supabase, CallSession, PerformanceMetrics } from "../utils/supabase-client.js";

export const getCallDetailsTool: Tool = {
...
export async function getCallDetails(args: { sessionId: string }): Promise<string> {
  ...
  const metrics = metricsData as PerformanceMetrics | null;
  ...
  if (metrics) {
    ...
    if (metrics.key_moments && metrics.key_moments.length > 0) {
      response += `**Key Moments:**\n${metrics.key_moments.map((km: any) => 
        `- ${km.timestamp}s: ${km.description} (${km.type})`
      ).join("\n")}\n\n`;
    }
  } else {

...
import { supabase, UserProgress } from "../utils/supabase-client.js";
...
export async function getUserProgress(args: { userId: string }): Promise<string> {
  const { userId } = args;
  ...
  const progress = data[0] as UserProgress;
  ...
  if (progress.average_score) {
    ...
    if (progress.average_score >= 80) performanceLevel = "Expert";
    else if (progress.average_score >= 70) performanceLevel = "Advanced";
    else if (progress.average_score >= 60) performanceLevel = "Intermediate";
